...
func main() {
	...
	flag.StringVar(&cfg.protocol, "protocol", "linux", "Boot protocol")
...
func computePlan(cfg config) (partitionPlan, error) {
	...
	plan.sysSizeMiB = (rootfsMiB * 110) / 100
	if plan.sysSizeMiB < minSysMiB {
		plan.sysSizeMiB = minSysMiB
	}
...
func buildEFIImage(cfg config) ([]byte, error) {
	...
	limineConf := buildLimineConfig(cfg.protocol, cfg.kargs, avDevice, rootDevice)
...
func buildLimineConfig(protocol, kargs, avDevice, rootDevice string) []byte {
	if protocol == "avyos" {
		return []byte("timeout: 0\n\n/AvyOS\n    protocol: limine\n    path: boot():/kernel\n")
	}
	cmdline := []string{
		"root=" + rootDevice,
		"rootfstype=btrfs",
	...
	return []byte(fmt.Sprintf("timeout: 0\n\n/AvyOS\n    protocol: %s\n    path: boot():/kernel\n    cmdline: %s\n    module_path: boot():/initrd\n", protocol, strings.Join(cmdline, " ")))
}
