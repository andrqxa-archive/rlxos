...
func (m *shellSessionManager) Open(owner uint32, sess *authSession, req dbgapi.ShellOpenRequest) (dbgapi.ShellSession, error) {
	if sess == nil || sess.Identity == nil {
		return dbgapi.ShellSession{}, errors.New("invalid auth session")
	}

	workdir, err := resolveWorkDir(sess.Identity, req.Cwd)
	if err != nil {
		return dbgapi.ShellSession{}, fmt.Errorf("failed to resolve workdir: %w", err)
	}
	...
	if err != nil {
		return dbgapi.ShellSession{}, fmt.Errorf("open pty: %w", err)
	}
	if err := ptyPair.SetSize(rows, cols); err != nil {
		_ = ptyPair.Close()
		return dbgapi.ShellSession{}, fmt.Errorf("set pty size: %w", err)
	}

	shellPath, err := resolveUserShell(sess)
	if err != nil {
		_ = ptyPair.Close()
		return dbgapi.ShellSession{}, err
	}
	...
	if err := cmd.Start(); err != nil {
		_ = ptyPair.Close()
		return dbgapi.ShellSession{}, fmt.Errorf("start shell: %w", err)
	}
	...
	return dbgapi.ShellSession{SessionID: sessionID}, nil
}

func (m *shellSessionManager) Input(owner uint32, sess *authSession, req dbgapi.ShellInputRequest) error {
...
func (m *shellSessionManager) Resize(owner uint32, sess *authSession, req dbgapi.ShellResizeRequest) error {
	session, err := m.sessionFor(owner, sess, req.SessionID)
	if err != nil {
		return err
	}
...
func (m *shellSessionManager) Close(owner uint32, sess *authSession, req dbgapi.ShellCloseRequest) error {
	session, err := m.sessionFor(owner, sess, req.SessionID)
	if err != nil {
		return err
	}
...
func (m *shellSessionManager) streamOutput(session *shellSession) {
	...
	for {
		...
		if n > 0 && m.service != nil {
			data := make([]byte, n)
			copy(data, buf[:n])
			_ = EmitShellOutput(m.service, session.owner, dbgapi.ShellOutputEvent{
...
func (m *shellSessionManager) waitProcess(session *shellSession) {
	...
	if m.service != nil {
		_ = EmitShellExit(m.service, session.owner, dbgapi.ShellExitEvent{
			SessionID: session.id,
			ExitCode:  exitCode,
		})
	}
...
func resolveUserShell(sess *authSession) (string, error) {
	shell := strings.TrimSpace(sess.Identity.Shell)
	if shell == "" {
