...
func cmdInfo(args []string) error {
	...
	dev, err := client.GetDevice(args[0])
	if err != nil {
...
func cmdMonitor(args []string) error {
	...
	client.OnDeviceAdded(func(ev ueventapi.DeviceEvent) {
		fmt.Printf("%s  %s  [%s]  %s\n", format.Color(format.Green, "ADD"), ev.DevPath, ev.Subsystem, ev.DevName)
	})
	client.OnDeviceRemoved(func(ev ueventapi.DeviceEvent) {
		fmt.Printf("%s  %s  [%s]  %s\n", format.Color(format.Red, "REMOVE"), ev.DevPath, ev.Subsystem, ev.DevName)
	})
	client.OnDeviceChanged(func(ev ueventapi.DeviceEvent) {
...
func cmdTrigger(args []string) error {
	...
	if err := client.Trigger(*subsystem); err != nil {
