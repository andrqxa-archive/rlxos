...
func cmdStart(args []string) error {
	...
	return runServiceAction(fs.Args(), "start", func(c *serviceapi.Client, name string) error { return c.Start(name) })
}
...
func cmdStop(args []string) error {
	...
	return runServiceAction(fs.Args(), "stop", func(c *serviceapi.Client, name string) error { return c.Stop(name) })
}
...
func cmdRestart(args []string) error {
	...
	return runServiceAction(fs.Args(), "restart", func(c *serviceapi.Client, name string) error { return c.Restart(name) })
}

func runServiceAction(args []string, verb string, fn func(*serviceapi.Client, string) error) error {
	if len(args) < 1 {
...
func stateOf(s serviceapi.ServiceStatus) string {
	switch {
	case s.Running:
		return "running"
	case s.Failed:
