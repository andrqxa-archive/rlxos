...
func PointInRoundedRectAt(px, py float64, r graphics.Rect, radius int) bool {
	if radius <= 0 {
...
func PointInRoundedRectAtClamped(px, py float64, r graphics.Rect, radius int) bool {
	if px < float64(r.X) || px >= float64(r.X+r.W) || py < float64(r.Y) || py >= float64(r.Y+r.H) {
		return false
	}
	if radius <= 0 {
...
func RoundedRectCoverage(x, y int, r graphics.Rect, radius int) float64 {
	if radius <= 0 {
		if r.ContainsXY(x, y) {
			return 1
		}
...
func ClampRoundedRadius(r graphics.Rect, radius int) int {
	if radius < 0 {
		return 0
	}
	if radius > r.W/2 {
...
func RoundedRectSignedDistance(px, py float64, r graphics.Rect, radius int) float64 {
	if r.W <= 0 || r.H <= 0 {
		return 1
	}
	if radius <= 0 {
