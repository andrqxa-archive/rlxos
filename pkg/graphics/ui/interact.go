...
func textInputSliceWidth(font *graphics.Font, display []rune, start, end int) int {
	if font == nil {
		return 0
	}
	if start < 0 {
...
func textInputVisibleRange(font *graphics.Font, display []rune, start, maxWidth int) (int, int) {
	if start < 0 {
		start = 0
	}
	if start > len(display) {
...
func textInputCursorFromX(font *graphics.Font, display []rune, start, end, relX int) int {
	if relX <= 0 || start >= end {
		return start
	}
	prevW := 0
...
func textAreaLineSliceWidth(font *graphics.Font, line []rune, start, end int) int {
	if font == nil {
		return 0
	}
	if start < 0 {
...
func textAreaVisibleEndCol(font *graphics.Font, line []rune, start, maxWidth int) int {
	if start < 0 {
		start = 0
	}
	if start > len(line) {
...
func textAreaCursorFromX(font *graphics.Font, line []rune, start, relX, maxWidth int) int {
	if start < 0 {
		start = 0
	}
	if start > len(line) {
...
func textAreaWrappedSegments(lines [][]rune, font *graphics.Font, maxWidth int) []textAreaWrapSegment {
	if len(lines) == 0 {
