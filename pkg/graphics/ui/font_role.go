...
var (
	elementFontCache sync.Map // key -> *graphics.Font
...
func elementFont(e *Element, fallbackRole string) *graphics.Font {
	if f := elementCustomFont(e, fallbackRole); f != nil {
		return f
	}
	return graphics.UIFont(resolveTextRole(e, fallbackRole))
}

func elementTitleFont(e *Element) *graphics.Font {
	return graphics.UIFont(resolveTitleRole(e))
}

func elementCustomFont(e *Element, fallbackRole string) *graphics.Font {
	if e == nil {
		return nil
	}
	...
	if size <= 0 {
		if base := graphics.UIFont(resolveTextRole(e, fallbackRole)); base != nil && base.Height > 0 {
	...
	if cached, ok := elementFontCache.Load(key); ok {
		if f, ok := cached.(*graphics.Font); ok && f != nil {
			return f
		}
	}

	font, err := graphics.LoadTTFFontFile(path, &gfxfont.Options{Size: size})
	if err != nil || font == nil {
