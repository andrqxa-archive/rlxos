...
func loadStdLibrary(registry *ComponentRegistry) error {
	for path, src := range stdSources {
		doc, err := Parse(src)
		if err != nil {
			return &BuildError{Message: "failed to parse standard library " + path + ": " + err.Error()}
		}
	...
	if err != nil {
		return &BuildError{Message: "failed to read embedded widgets: " + err.Error()}
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			return &BuildError{Message: "failed to read embedded widget " + path + ": " + err.Error()}
		}
		src := string(data)
		stdSources[path] = src
		...
		doc, err := Parse(src)
		if err != nil {
			return &BuildError{Message: "failed to parse embedded widget " + path + ": " + err.Error()}
		}
		for _, comp := range doc.Components {
			registry.Register(comp)
		}
