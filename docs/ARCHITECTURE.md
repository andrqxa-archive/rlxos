...
- writable **state + apps** mounted at `/config`, `/apps`, `/users`, `/cache`
- a decentralized IPC mechanism (**Sutra**) used for service↔client communication
- a graphics stack implemented as a service (`display`) and a client framework (`pkg/graphics`)
...
- executing the OS core init (`/avyos/cmd/init`)
...
- `/avyos/cmd/`        Core utilities and system entrypoints
- `/avyos/services/`   Long-running daemons
- `/avyos/apps/`       OS-shipped apps (optional)
- `/avyos/config/`     Factory defaults / fallback configuration
- `/avyos/data/`       OS-shipped shared data (usr-share-like)
...
- `/cache/kernel/processes`  Process information (proc-equivalent)
- `/cache/kernel/sysfs`      sysfs-equivalent
- `/cache/kernel/devices`    device nodes (devtmpfs-equivalent)
...
Services are started and supervised by the init/service-management layer in `/avyos/cmd/`.
...
The OS core ships CLI tools in `/avyos/cmd/` which implement system-facing functionality such as:
- init (PID 1)
- service management
...
### 9.2 Graphics framework (`pkg/graphics`)
`pkg/graphics` provides the client-side graphics framework:
...
- distro environment is created/managed by `cmd/distro`
